...
function PreencherCombosSetor() {
  ...
  const baseUrl = "https://ssge.onrender.com/responsavel-setor/combos";
  ...
  const abrirModal = async (combo) => {
    ...
    try {
      ...
      const resPreench = await axios.get(`https://ssge.onrender.com/responsavel-setor/preenchimentos/${combo.id}`, makeConfig());
  ...
  const enviarPreenchimento = async () => {
    ...
    try {
      await axios.put(`https://ssge.onrender.com/responsavel-setor/preenchimentos/${comboSelecionado.id}`, payload, makeConfig());
