...
  // NOVO ESTADO: Para controlar o texto do botão Salvar/Atualizar
  const [preenchimentoExiste, setPreenchimentoExiste] = useState(false); 

  const token = localStorage.getItem('token');
  const makeConfig = () => ({ headers: { Authorization: `Bearer ${token}` } });
  const baseUrl = "http://localhost:8081/responsavel-setor/combos";

  // Função principal de busca (reutilizável)
  const buscarCombos = async (idSetor = setorSelecionado) => {
    try {
      let url = baseUrl;
...
      console.error("Erro em buscarCombos:", error);
    }
  };
  
  useEffect(() => {
...
        
        // Carrega combos iniciais (pode usar buscarCombos aqui)
...
  const filtrarPorSetor = (idSetor) => {
    setSetorSelecionado(idSetor);
    buscarCombos(idSetor);
  };
...
    setIsModalOpen(true);
    setItens([]);
    // Reseta o status de preenchimento para garantir que o texto do botão seja carregado corretamente
    setPreenchimentoExiste(false); 
    
    try {
      // A. Busca Itens
...
        // Atualiza o estado: se encontrou dados, mostra 'Atualizar' no botão
        setPreenchimentoExiste(true); 
        
        resPreench.data.forEach(p => {
            // Os dados preenchidos anteriormente já estão sendo carregados aqui
            novosValores[p.itemId] = p.valor;
            novasQtds[p.itemId] = p.quantidade; 
            novasObs[p.itemId] = p.observacao || "";
        });
      } else {
        setPreenchimentoExiste(false); // Garante que o texto seja 'Salvar'
      }
...
    } catch (error) {
      toast.error("Erro ao carregar detalhes ou preenchimentos anteriores.");
      console.error(error);
      setPreenchimentoExiste(false);
    }
  };
...
    setQtds({});
    setObservacoes({});
    setPreenchimentoExiste(false); // Reseta o estado
  };
...
        itemId: item.id,
        // Garante que valores nulos/vazios sejam 0 ou nulo, dependendo do backend
...
    try {
      // Ação de PUT (Atualizar) já está correta
      await axios.put(`http://localhost:8081/responsavel-setor/preenchimentos/${comboSelecionado.id}`, payload, makeConfig());
      
      // Atualiza a mensagem baseado na ação
      const acao = preenchimentoExiste ? "atualizado" : "salvo";
      toast.success(`Preenchimento ${acao} com sucesso!`);
      
      // Atualiza a lista de combos para refletir a mudança (se necessário)
      buscarCombos();
      fecharModal();
    } catch (error) {
      toast.error("Erro ao salvar/atualizar preenchimento.");
      console.error(error);
    } finally {
      setEnviando(false);
    }
